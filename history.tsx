        ...
        {selectedSession?.workerId ? <div className="h-[58vh] min-h-0 overflow-hidden rounded-lg border border-border">
          ...
          {members.map(({ member, input, inputVerified }) => <section key={member.key} className="space-y-3">
            <article className="mr-auto max-w-[95%] rounded-xl border border-border bg-card p-4">
              ...
              {member.fileGateway !== undefined && <p className="mt-2 text-xs text-muted-foreground">{member.fileGateway ? t("File Gateway was available for this attempt. Actual reads appear in the participant session.", "File Gateway был доступен для этой попытки. Фактические чтения видны в сессии участника.") : t("External file access was unavailable for this attempt.", "Внешнее чтение было недоступно для этой попытки.")}</p>}
