...
In a new-chat composer, click the MoA chip and enable the pair before sending. Hidden draft metadata binds MoA to the first message. Turning MoA off in that panel removes the metadata. Before the main workspace exists the advisor uses the project source on the resolved existing host (personal workspace only for an unfiled chat).
